...
def predict(pred_data, model, buffer, gpu, shrink_pixel):
    """
    Predict by tile
    Params:
        pred_data (''DataLoader''): Batch grouped data
        model: Trained model for prediction
        scorePath (str): Directory relative to s3 bucket to save probability 
            maps
        predPath (str): Directory relative to s3 bucket to save hard prediction 
            maps
    ...
    for img, index_batch in pred_data:
        ...
        for i in range(batch):
            ...
            for n in range(nclass - 1):
                ...
                try:
                    ...
                    canvas_score_ls[n][
                        index[0] + buffer * (index[0] != 0):index[0] + buffer \
                            * (index[0] != 0) + score_height,
                        index[1] + buffer * (index[1] != 0):index[1] + buffer \
                            * (index[1] != 0) + score_width
                    ] = out_score
...
def mc_predict(pred_data, model, num_mc_trials, buffer, gpu, shrink_pixel):
    """
            ...
            for i in range(batch):
                ...
                for n in range(nclass - 1):
                    out_score = out[
                        ...
                        (index[0] != 0) * buffer:(index[0] != 0) * buffer \
                            + chip_height \
                                + (index[0] == 0 or index[0] == max_index_0) \
                                    * buffer,
                        (index[1] != 0) * buffer: (index[1] != 0) * buffer \
                            + chip_height \
    ...
    for i in range(len(list(mc_score_dict.values())[0])):
        mc_canvas_score_ls.append(
            np.concatenate(
                [np.expand_dims(value_ls[i], 0) 
                 for value_ls in mc_score_dict.values()], 
                0
            )
        )
