...
class ConvBlock(nn.Module):
    ...
    Args:
        ...
        stride : int 
            decides how jumpy kernel moves along the spatial dimensions.
        padding : int 
            how much the input should be padded on the borders with zero.
        dilation : int
            dilation ratio for enlarging the receptive field.
        num_conv_layers : int 
            Number of conv+BN+ReLU layers in the block.
        drop_rate : float 
            dropout rate at the end of the block.
    """
...
class UpconvBlock(nn.Module):
    ...
    Args:
        ...
        out_channels : int
            number of output features.
        upmode : str
            Upsampling type. If "fixed" then a linear upsampling with scale 
            factor of two will be applied using bi-linear as interpolation 
            method. If deconv_1 is chosen then a non-overlapping transposed 
            convolution will be applied to upsample the feature maps. 
            If deconv_1 is chosen then an  overlapping transposed convolution 
            will be applied to upsample the feature maps.
    """
...
class AdditiveAttentionBlock(nn.Module):
    ...
    Args:
        f_g : int
            number of feature maps collected from the higher resolution in encoder path.
        f_x : int
            number of feature maps in layer "x" in the decoder.
        f_inter : int
            number of feature maps after summation equal to the number of
