...
def evaluate(eval_data='', model='', buffer='', gpu='', bucket=None, 
             out_prefix='', filename=None):
    """
    Evaluate model
    Params:
        eval_data (''DataLoader''): Batch grouped data
        model: Trained model for validation
        buffer: Buffer added to the targeted grid when creating dataset. This 
            allows metrics to calculate only at non-buffered region
        gpu (binary,optional): Decide whether to use GPU, default is True
        bucket (str): name of s3 bucket to save metrics
        out_prefix (str): s3 prefix to save metrics
    """
    ...
    if not filename:
        filename = "Metrics"
    dir_metrics = f"{out_prefix}/{filename}.csv"
    #dir_metrics = f"s3://{bucket}/{out_prefix}/{filename}.csv"
